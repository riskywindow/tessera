...
| R-3 | A newer driver (PyTorch wheels ship their own cudart) returns symbol variants this build's `cuda.h` 12.6 does not know. | Launches pass through unhooked and uncounted. | WP-0.5 | Identify variants by pointer identity, pass unknown ones through, and expose `bypassed_lookups` so the gap is visible rather than silent. | H1 | open |
